            ...
            if (!this.CheckLogin(out msg))
            {
                ...
                else
                {
                    filterContext.Result = Content(string.Concat("<script>",
                        ...
                        string.Compare(filterContext.Controller.ToString(), "WebMvc.Controllers.HomeController", true) == 0 ? "top.location='" + Url.Content("~/Login/Login") + "'" : "top.login();", "</script>"), "text/html");
                }
            }
