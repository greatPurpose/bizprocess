    ...
    this.init = function ($comboxs)
    {
        $comboxs.each(function ()
        {
            ...
            var $div = $('<div id="' + id + '_selectdiv" style="left:' +
                (BPUI.Core.isIe6Or7() ? $(this).get(0).offsetLeft + 5 : $(this).get(0).offsetLeft + 6) + 'px;width:' +
                (width1 ? width1 + "px" : "auto") + ';height:' + height1 + 'px;white-space:nowrap;' +
                (BPUI.Core.isIe6Or7() ? 'top:' + ($(this).get(0).offsetTop + 37) + 'px' : '')
                + '" class="comboxdiv"></div>');
            if (readonly)
            {
            ...
            switch (tagName)
            {
                ...
                case "span":
                    ...
                    //new BPUI.Grid({ table: $table, height: $div.height(), resizeCol:false });
                    break;
            }
