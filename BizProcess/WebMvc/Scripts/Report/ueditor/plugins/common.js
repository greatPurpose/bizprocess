...
function getTables(connid)
{
    var tables = [];
    $.ajax({
        url: top.rootdir + "/Report/GetTables?connid=" + connid, dataType: "json", async: false, cache: false, success: function (json)
        {
...
function getFields(connid, table)
{
    var fields = [];
    $.ajax({
        url: top.rootdir + "/Report/GetFields?connid=" + connid + "&table=" + table, dataType: "json", async: false, cache: false, success: function (json)
        {
            for (var i = 0; i < json.length; i++)
            {
                fields.push(json[i]);
...
function testSql(sql)
{
    ...
    var json = parent.reportattributeJSON;
    if (!json || !json.dbconn)
    {
        alert("未设置数据连接");
        return;
...
function setEvents(id)
{
    ...
    var reportEvents = parent.reportEvents;
    var isIn = false;
    for (var i = 0; i < reportEvents.length; i++)
    {
        if (reportEvents[i].id == id)
        {
            reportEvents[i].events = events;
            isIn = true;
            break;
        }
    }
    if (!isIn)
    {
        reportEvents.push({ "id": id, "events": events });
    }
}
...
function getEvents(id)
{
    ...
    for (var i = 0; i < reportEvents.length; i++)
    {
        if (reportEvents[i].id == id)
        {
            if (reportEvents[i].events.length > 0)
            {
                $("#event_name").val(reportEvents[i].events[0].name);
                $("#event_script").val(reportEvents[i].events[0].script);
            }
            return reportEvents[i].events;
            break;
        }
    }
    return [];
