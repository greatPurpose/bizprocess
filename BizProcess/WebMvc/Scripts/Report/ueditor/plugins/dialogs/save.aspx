...
<body style="overflow:hidden;">
    ...
    <script type="text/javascript">
        ...
        function save()
        {
            ...
            if (!reportattributeJSON.name || $.trim(reportattributeJSON.name).length == 0)
            {
                if (!isPublish)
                {
                    alert("您未设置表单相关属性!");
            ...
            var reportid = "";
            if (!reportattributeJSON.id || $.trim(reportattributeJSON.id).length == 0)
            {
                reportid = BPUI.Core.newid()
                reportattributeJSON.id = reportid;
            }
            else
            {
                reportid = reportattributeJSON.id;
            }

            $.ajax({
                url: top.rootdir + "/Report/Save", type: "POST",
                data: {
                    id: reportid,
                    html: html,
                    name: reportattributeJSON.name,
                    type: reportattributeJSON.apptype,
