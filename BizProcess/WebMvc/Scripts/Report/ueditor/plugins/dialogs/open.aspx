...
<%
    ...
    string rootid = new BizProcess.Platform.Dictionary().GetIDByCode("FormTypes").ToString();
            ...
            <td style="vertical-align:top; padding:2px 0 0 3px; border-left:1px solid #cccccc;">
                <div id="ListDiv" style="overflow:auto;">
                    ...
                    <script type="text/javascript">
                        ...
                        {
                            $.ajax({
                                url: top.rootdir + "/Report/GetHtml?id=" + id, async: false, cache: false,
                            ...
                            $.ajax({
                                url: top.rootdir + "/Report/GetAttribute?id=" + id, dataType: "JSON", async: false, cache: false,
                                success: function (json)
                                {
                                    parent.reportattributeJSON = json;
                                }
                            });
/*                            $.ajax({
                                url: top.rootdir + "/Report/Getsubtable?id=" + id, dataType: "JSON", async: false, cache: false,
                                success: function (json)
                                {
  ...
  */                          dialog.close();
    ...
    <script type="text/javascript">
        ...
        $(function ()
        {
            ...
            roadTree = new BPUI.Tree({ id: "TypeDiv", path: top.rootdir + "/Dict/Tree1?root=<%=rootid%>", refreshpath: top.rootdir + "/Dict/TreeRefresh", onclick: openUrl });
            openUrl({ id: "<%=rootid%>" });
        });
