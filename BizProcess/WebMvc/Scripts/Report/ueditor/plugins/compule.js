UE.compule = {
    ...
    getRadioOrCheckboxHtml: function ($control, type)
    {
        ...
        if ("0" == datasource)//数据字典
        {
            ...
            if ('radio' == type)
            {
                radios = '<span>@Html.Raw(BDictionary.GetRadiosByID("' + dictid + '".ToGuid(), "' + id + '", BizProcess.Platform.Dictionary.OptionValueField.ID, "' + defaultvalue + '", "isflow=\'1\' type1=\'flow_radio\'' + eventArrs + '"))' + eventScripts + '</span>';
            }
            else if ('checkbox' == type)
            {
                radios = '<span>@Html.Raw(BDictionary.GetCheckboxsByID("' + dictid + '".ToGuid(), "' + id + '", BizProcess.Platform.Dictionary.OptionValueField.ID, "' + defaultvalue + '", "isflow=\'1\' type1=\'flow_checkbox\'' + eventArrs + '"))' + eventScripts + '</span>';
            }
        ...
        else if ("2" == datasource)//SQL
        {
            ...
            if ("radio" == type)
            {
                radios += '<span>@Html.Raw(new BizProcess.Platform.ReportTemplate().GetRadioFromSql(DBConnID, "' + sql.replaceAll('"', '\"') + '", "' + id + '", "' + defaultvalue + '", "isflow=\'1\' type1=\'flow_radio\'' + eventArrs + '"))' + eventScripts + '</span>';
            }
            else if ("checkbox" == type)
            {
                radios += '<span>@Html.Raw(new BizProcess.Platform.ReportTemplate().GetCheckboxFromSql(DBConnID, "' + sql.replaceAll('"', '\"') + '", "' + id + '", "' + defaultvalue + '", "isflow=\'1\' type1=\'flow_checkbox\'' + eventArrs + '"))' + eventScripts + '</span>';
            }
            $control.after(radios);
            $control.remove();
        }
    ...
    getSelectHtml: function ($control)
    {
        ...
        if ("0" == datasource)//数据字典
        {
            ...
            radios += '@Html.Raw(BDictionary.GetOptionsByID("' + dictid + '".ToGuid(), BizProcess.Platform.Dictionary.OptionValueField.ID, "' + defaultvalue + '"))';
            radios += '</select>';
            var $radios = $(radios);
        ...
        else if ("2" == datasource)//SQL
        {
            ...
            radios += '@Html.Raw(new BizProcess.Platform.ReportTemplate().GetOptionsFromSql(DBConnID, "' + sql.replaceAll('"', '\"') + '", "' + defaultvalue + '"))';
            radios += '</select>';
            var $radios = $(radios);
            $radios.attr("eventsid", $control.attr("eventsid"));
            $control.after($radios);
    ...
    getOrgHtml: function ($control)
    {
        ...
        switch (org_rang)
        {
            case "0": //发起者部门
                ...
                break;
            case "1": //处理者部门
                rootid = '@BizProcess.Platform.Users.CurrentDeptID';
                break;
            case "2": //自定义
    ...
    {
        reportattributeJSON = json || parent.reportattributeJSON;
        reportattributeJSON.hasEditor = "1";
        var id = $control.attr("id");
        var table = '', field = '';
    ...
    getGridHtml: function ($control)
    {
        ...
        div += '" >';
        div += '@Html.Raw(new BizProcess.Platform.ReportTemplate().GetFormGridHtml(DBConnID, "' + $control.attr("dataformat") + '","' + $control.attr("datasource") + '","' + $control.attr("datasource1") + '"))';
        div += '</div>';
        $control.after(div);
        $control.remove();
    },
    ...
    getSubTableHtml_Select: function (colnumJSON, id, i, iscount)
    {
        ...
        switch (ds)
        {
            case "select_dsdict":
                ...
                select += '@Html.Raw(BDictionary.GetOptionsByID("' + rootid + '".ToGuid(), BizProcess.Platform.Dictionary.OptionValueField.ID, "' + dvalue + '"))';
                break;
            case "select_dssql":
                ...
                select += '@Html.Raw(new BizProcess.Platform.ReportTemplate().GetOptionsFromSql(DBConnID, "' + sql.replaceAll('"', '\"') + '", "' + dvalue + '"))';
                break;
    ...
    getSubTableHtml_Checkbox: function (colnumJSON, id, i, iscount)
    {
        ...
        switch (ds)
        {
            case "checkbox_dsdict":
                ...
                checkbox = '<span>@Html.Raw(BDictionary.GetCheckboxsByID("' + rootid + '".ToGuid(), "' + name + '", BizProcess.Platform.Dictionary.OptionValueField.ID, "' + dvalue + '", "issubflow=\'1\' type1=\'subflow_checkbox\' colname=\'' + colnumJSON.name + '\'"))</span>';
                break;
            case "checkbox_dssql":
                var sql = editmode.checkbox_ds_sql;
                checkbox = '@Html.Raw(new BizProcess.Platform.ReportTemplate().GetCheckboxFromSql(DBConnID, "' + sql.replaceAll('"', '\"') + '", "' + name + '", "' + dvalue + '", "issubflow=\'1\' type1=\'subflow_checkbox\'" colname=\'' + colnumJSON.name + '\'"))';
                break;
    ...
    getSubTableHtml_Radio: function (colnumJSON, id, i, iscount)
    {
        ...
        switch (ds)
        {
            case "radio_dsdict":
                ...
                radio = '<span>@Html.Raw(BDictionary.GetRadiosByID("' + rootid + '".ToGuid(), "' + name + '", BizProcess.Platform.Dictionary.OptionValueField.ID, "' + dvalue + '", "issubflow=\'1\' type1=\'subflow_radio\' colname=\'' + colnumJSON.name + '\'"))</span>';
                break;
            case "radio_dssql":
                var sql = editmode.radio_ds_sql;
                radio = '@Html.Raw(new BizProcess.Platform.ReportTemplate().GetRadioFromSql(DBConnID, "' + sql.replaceAll('"', '\"') + '", "' + name + '", "' + dvalue + '", "issubflow=\'1\' type1=\'subflow_radio\'" colname=\'' + colnumJSON.name + '\'"))';
                break;
            case "radio_dsstring":
    ...
    getSubTableHtml: function ($control)
    {
        ...
        var guid = BPUI.Core.newid(false);
