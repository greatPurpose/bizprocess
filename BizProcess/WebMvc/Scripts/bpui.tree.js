...
;BPUI.Tree = function (options)
{
    ...
    this.init = function ()
    {
        ...
        //try
        //{
        //    var contextMenu = new BPUI.Menu({ srcElement: this.$srcObj });
    ...
    this.addNodes = function (json, $node, isRefresh, isInit)
    {
        ...
        {
            var jsoni = json[i];
            var id = jsoni.id || BPUI.Core.newid(false);
            ...
            var $div = $('<div id="bpui_tree_' + id + '" class="tree_div"></div>');
        ...
        for (var i = 0; i < json.length; i++)
        {
            ...
            {
                this.addNodes(jsoni, $("#bpui_tree_" + jsoni.id, $rootObj), isRefresh, false);
            }
        }
    };
    ...
    this.toggleNode = function ($ul, isRefresh)
    {
        ...
        if (isRefresh || $childNodes.size() == 0)
        {
            ...
            var id = $div.attr("id").replace("bpui_tree_", "");
