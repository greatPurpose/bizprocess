formrun = {
        ...
        switch (type)
        {
            ...
            case "subflow_org":
                if (filedshow == 0)
                {
                    ...
                    {
                        new BPUI.Member().setValue($control.prev());
                    }
                }
                else if (filedshow == 1)
                {
            ...
            case "subflow_dict":
                ...
                {
                    $control.prev().val(initValue);
                    new BPUI.Dict().setValue($control.prev());
                }
                else if (filedshow == 1)
                {
                    $control.prev().val(initValue);
    ...
    subtableNewRow: function (but, pkValue, isAppend, isLoad)//pkValue：主键值 isAppend 0：为在当前行后增加 1：为在最后一行增加 isLoad:是否为二次加载
    {
        ...
        var $newtr = $tr.clone();
        var guid = pkValue || BPUI.Core.newid(false);
        ...
        $("[type1='subflow_files']", $newtr).each(function ()
        {
            ...
            new BPUI.File().init($(this));
        });
        $("input[name^='hidden_guid_']", $newtr).val(guid);
        new BPUI.Text().init($(".mytext", $newtr));
        new BPUI.Button().init($("input[type='button']", $newtr));
    ...
    subtableCount: function (id, colname, showid, isReadonly)//从表计算合计 id:从表ID, colname:列, showid：显示合计值的label id, isReadonly:是否为只读状态
    {
        ...
        for (var i = 0; i < $tds.size() ; i++)
        {
            ...
            if (val && !isNaN(val))
            {
                count = BPUI.Core.accAdd(count, parseFloat(val));
            }
        }
