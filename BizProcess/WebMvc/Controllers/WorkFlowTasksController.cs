...
namespace WebMvc.Controllers
{
    public class WorkFlowTasksController : MyController
    {
        ...
        public ActionResult Index(FormCollection collection)
        {
            ...
            string query = string.Format("&appid={0}&tabid={1}&title={2}&flowid={3}&sender={4}&date1={5}&date2={6}&type={7}",
                Request.QueryString["appid"], Request.QueryString["tabid"], title.UrlEncode(), flowid, sender, date1, date2, type);
            string pager;

            var taskList = bworkFlowTask.GetTasks(BizProcess.Platform.Users.CurrentUserID,
               out pager, query, title, flowid, sender, date1, date2, type);
            ViewBag.query = query;
        ...
        public string GetTaskStepDetails()
        {
            ...
            List<BizProcess.Data.Model.WorkFlowTask> tasks = bworkFlowTask.GetTaskList(flowid.ToGuid(), stepid.ToGuid(), groupid.ToGuid());
            ...
            if (tasks.Count > 0)
            {
                ...
                retJson = "{";
                retJson += string.Format("\"flowid\":\"{0}\",", tasks[0].FlowID);
                retJson += string.Format("\"stepid\":\"{0}\",", tasks[0].StepID);
                retJson += string.Format("\"stepname\":\"{0}\",", tasks[0].StepName);
                retJson += string.Format("\"groupid\":\"{0}\",", tasks[0].GroupID);
                retJson += string.Format("\"instanceid\":\"{0}\",", tasks[0].InstanceID);
                retJson += string.Format("\"taskid\":\"{0}\"", tasks[0].ID);
                retJson += "}";
            }
            return retJson;
        }
        ...
        public ActionResult Designate(FormCollection collection)
        {
            ...
            if (taskid.IsGuid(out taskID))
            {
                ...
                BizProcess.Platform.WorkFlowTask btask = new BizProcess.Platform.WorkFlowTask();
                var users = new BizProcess.Platform.Organize().GetAllUsers(user);
                System.Text.StringBuilder sb = new System.Text.StringBuilder();
                foreach (var user1 in users)
                {
                    btask.DesignateTask(taskID, user1);
                    BizProcess.Platform.Log.Add("管理员指派了流程任务", "将任务" + taskID + "指派给了：" + user1.Name + user1.ID, BizProcess.Platform.Log.Types.流程相关);

                    sb.Append(user1.Name);
                    sb.Append(",");
                }
                string userNames = sb.ToString().TrimEnd(',');
                ViewBag.Script = "alert('已成功指派给：" + userNames + "!');new BPUI.Window().reloadOpener();new BPUI.Window().close();";
            }
            return View();
        }
        ...
        public string Delete()
        {
            ...
            if (flowid.IsGuid(out fid) && groupid.IsGuid(out gid))
            {
                ...
                var tasks = new BizProcess.Platform.WorkFlowTask().GetTaskList(fid, gid);
                foreach (var task in tasks)
                {
                    delxml.Append(task.Serialize());
                }
                new BizProcess.Platform.WorkFlowTask().DeleteInstance(fid, gid);
                BizProcess.Platform.Log.Add("管理员删除了流程实例", delxml.ToString(), BizProcess.Platform.Log.Types.流程相关);
                return "删除成功!";
            }
            else
            {
        ...
        public string Withdraw()
        {
            ...
            else if (new BizProcess.Platform.WorkFlowTask().HasWithdraw(tid))
            {
                bool success = new BizProcess.Platform.WorkFlowTask().WithdrawTask(tid);
                if (success)
                {
                    BizProcess.Platform.Log.Add("收回了任务", "任务ID：" + taskid, BizProcess.Platform.Log.Types.流程相关);
                    return "收回成功!";
                }
                else
                {
