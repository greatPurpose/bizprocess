...
namespace WebMvc.Controllers
{
    public class DictController : MyController
    {
        ...
        { 
            ...
            return new BizProcess.Platform.Dictionary().HasCode(code, id) ? "唯一代码重复" : "1";
        }
        ...
        {
            BizProcess.Data.Model.Dictionary dict = new BizProcess.Data.Model.Dictionary();
            BizProcess.Platform.Dictionary bdict = new BizProcess.Platform.Dictionary();
            string id = Request.QueryString["id"];
            if (!id.IsGuid())
            {
            ...
            if (collection != null)
            {
                ...
                bdict.Add(dict);
                bdict.RefreshCache();
                BizProcess.Platform.Log.Add("添加了数据字典项", dict.Serialize(), BizProcess.Platform.Log.Types.数据字典);
