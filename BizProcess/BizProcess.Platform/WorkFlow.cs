...
namespace BizProcess.Platform
{
    ...
    {
        private BizProcess.Data.Interface.IWorkFlow dataWorkFlow;
        public WorkFlow()
        {
        ...
        /// 查询所有记录
        /// </summary>
        public List<BizProcess.Data.Model.WorkFlow> GetAll()
        {
        ...
        /// 查询单条记录
        /// </summary>
        public BizProcess.Data.Model.WorkFlow Get(Guid id)
        {
        ...
        public string SaveFlow(string jsonString)
        {
            ...
            else
            {
                ...
                BizProcess.Data.Model.WorkFlow wf = bwf.Get(flowID);
                bool isAdd = false;
                if (wf == null)
                {
                    wf = new BizProcess.Data.Model.WorkFlow();
                    isAdd = true;
                    wf.ID = flowID;
                    wf.CreateDate = BizProcess.Utility.DateTimeNew.Now;
                    wf.CreateUserID = BizProcess.Platform.Users.CurrentUserID;
                    wf.Status = 1;
                }
        ...
        public string InstallFlow(string jsonString, bool isMvc = true)
        {
            ...
            BizProcess.Data.Model.WorkFlowInstalled wfInstalled = GetWorkFlowRunModel(jsonString, out errMsg);
            if (wfInstalled == null)
            {
                return errMsg;
            }
            ...
            {
                BizProcess.Data.Model.WorkFlow wf = dataWorkFlow.Get(wfInstalled.ID);
                if (wf == null)
                {
                    return "流程实体为空";
                }
                else
                {
                    using (System.Transactions.TransactionScope scope = new System.Transactions.TransactionScope())
                    {
                        ...
                        #region 添加到应用程序库
                        BizProcess.Platform.AppLibrary bappLibrary = new AppLibrary();
                        BizProcess.Data.Model.AppLibrary app = bappLibrary.GetByCode(wfInstalled.ID.ToString());
                        bool isAdd = false;
                        if (app == null)
                        {
                            isAdd = true;
                        ...
                        #endregion
                        BizProcess.Cache.IO.Opation.Set(getCacheKey(wfInstalled.ID), wfInstalled);
                        scope.Complete();
                        return "1";
                    }
                }
        ...
        public BizProcess.Data.Model.WorkFlow SaveAs(Guid flowID, string newName)
        {
            BizProcess.Data.Model.WorkFlow wf = dataWorkFlow.Get(flowID);
        ...
        {
            return string.Concat(BizProcess.Utility.Keys.CacheKeys.WorkFlowInstalled_.ToString(), flowID.ToString("N"));
        }
        ...
        /// <param name="flowID">流程ID</param>
        /// <returns></returns>
        public BizProcess.Data.Model.WorkFlowInstalled GetWorkFlowRunModel(string flowID, bool cache = true)
        {
            Guid fid;
        ...
        /// <param name="flowID">流程ID</param>
        /// <returns></returns>
        public BizProcess.Data.Model.WorkFlowInstalled GetWorkFlowRunModel(Guid flowID, bool cache = true)
        {
            if (!cache)
            {
            ...
            else
            {
                ...
                if (obj == null)
                {
                    ...
                    BizProcess.Cache.IO.Opation.Set(key, wfi);
                    return wfi;
                }
                else
                {
                    return obj as BizProcess.Data.Model.WorkFlowInstalled;
                }
            }
  
        }

        private BizProcess.Data.Model.WorkFlowInstalled getWorkFlowRunFromDesign(Guid flowID)
        {
            var wf = Get(flowID);
        ...
        {
            string key = getCacheKey(flowID);
            BizProcess.Cache.IO.Opation.Set(key, GetWorkFlowRunModel(flowID, false));
        }
        ...
        public BizProcess.Data.Model.WorkFlowInstalled GetWorkFlowRunModel(string jsonString, out string errMsg)
        {
            errMsg = "";
            BizProcess.Data.Model.WorkFlowInstalled wfInstalled = new BizProcess.Data.Model.WorkFlowInstalled();
            ...
            wfInstalled.DebugUsers = new BizProcess.Platform.Organize().GetAllUsers(json["debugUsers"].ToString());
            wfInstalled.Note = json["note"].ToString();

            List<BizProcess.Data.Model.WorkFlowInstalledSub.DataBases> dataBases = new List<BizProcess.Data.Model.WorkFlowInstalledSub.DataBases>();
            var dbs = json["databases"];
            if (dbs.IsArray)
            {
                foreach (LitJson.JsonData db in dbs)
                {
                    dataBases.Add(new BizProcess.Data.Model.WorkFlowInstalledSub.DataBases()
                    {
            ...
            {
                wfInstalled.TitleField = new BizProcess.Data.Model.WorkFlowInstalledSub.TitleField()
                {
            ...
            #region 载入步骤信息
            List<BizProcess.Data.Model.WorkFlowInstalledSub.Step> stepsList = new List<BizProcess.Data.Model.WorkFlowInstalledSub.Step>();
            LitJson.JsonData steps = json["steps"];
            if (steps.IsArray)
            {
                foreach (LitJson.JsonData step in steps)
                {
                    ...
                    BizProcess.Data.Model.WorkFlowInstalledSub.StepSet.Behavior behavior1 = new BizProcess.Data.Model.WorkFlowInstalledSub.StepSet.Behavior();
                    ...
                    List<BizProcess.Data.Model.WorkFlowInstalledSub.StepSet.Button> buttionList = new List<BizProcess.Data.Model.WorkFlowInstalledSub.StepSet.Button>();
                    if (buttons.IsArray)
                    {
                        foreach (LitJson.JsonData button in buttons)
                        {
                            ...
                            buttionList.Add(new BizProcess.Data.Model.WorkFlowInstalledSub.StepSet.Button()
                            {
                                ID = butID,
                    ...
                    BizProcess.Data.Model.WorkFlowInstalledSub.StepSet.Event event2 = new BizProcess.Data.Model.WorkFlowInstalledSub.StepSet.Event();
                    if (event1.IsObject)
                    {
                    ...
                    List<BizProcess.Data.Model.WorkFlowInstalledSub.StepSet.Form> formList = new List<BizProcess.Data.Model.WorkFlowInstalledSub.StepSet.Form>();
                    if (forms.IsArray)
                    {
                        foreach (LitJson.JsonData form in forms)
                        {
                            formList.Add(new BizProcess.Data.Model.WorkFlowInstalledSub.StepSet.Form()
                            {
                    ...
                    List<BizProcess.Data.Model.WorkFlowInstalledSub.StepSet.FieldStatus> fieldStatusList = new List<BizProcess.Data.Model.WorkFlowInstalledSub.StepSet.FieldStatus>();
                    if (fieldStatus.IsArray)
                    {
                        foreach (LitJson.JsonData field in fieldStatus)
                        {
                            fieldStatusList.Add(new BizProcess.Data.Model.WorkFlowInstalledSub.StepSet.FieldStatus()
                            {
                    ...
                    stepsList.Add(new BizProcess.Data.Model.WorkFlowInstalledSub.Step()
                    {
            ...
            List<BizProcess.Data.Model.WorkFlowInstalledSub.Line> linesList = new List<BizProcess.Data.Model.WorkFlowInstalledSub.Line>();
            LitJson.JsonData lines = json["lines"];
            if (lines.IsArray)
            {
                foreach (LitJson.JsonData line in lines)
                {
                    linesList.Add(new BizProcess.Data.Model.WorkFlowInstalledSub.Line()
                    {
            ...
            if (wf != null)
            {
                ...
                wfInstalled.InstallTime = BizProcess.Utility.DateTimeNew.Now;
        ...
        /// <returns></returns>
        public List<BizProcess.Data.Model.WorkFlowInstalledSub.Step> GetAllPrevSteps(Guid flowID, Guid stepID)
        {
            List<BizProcess.Data.Model.WorkFlowInstalledSub.Step> stepList = new List<BizProcess.Data.Model.WorkFlowInstalledSub.Step>();
        ...
        private void addPrevSteps(List<BizProcess.Data.Model.WorkFlowInstalledSub.Step> list, BizProcess.Data.Model.WorkFlowInstalled wfInstalled, Guid stepID)
        {
        ...
        /// <param name="stepID"></param>
        /// <returns></returns>
        public List<BizProcess.Data.Model.WorkFlowInstalledSub.Step> GetPrevSteps(Guid flowID, Guid stepID)
        {
            List<BizProcess.Data.Model.WorkFlowInstalledSub.Step> stepList = new List<BizProcess.Data.Model.WorkFlowInstalledSub.Step>();
            var wfInstalled = GetWorkFlowRunModel(flowID);
            if (wfInstalled == null)
        ...
        /// <param name="stepID"></param>
        /// <returns></returns>
        public List<BizProcess.Data.Model.WorkFlowInstalledSub.Step> GetNextSteps(Guid flowID, Guid stepID)
        {
            List<BizProcess.Data.Model.WorkFlowInstalledSub.Step> stepList = new List<BizProcess.Data.Model.WorkFlowInstalledSub.Step>();
            var wfInstalled = GetWorkFlowRunModel(flowID);
            if (wfInstalled == null)
        ...
        /// <param name="defautFirstStepName">如果步骤ID为空是否默认为第一步</param>
        /// <returns></returns>
        public string GetStepName(Guid stepID, BizProcess.Data.Model.WorkFlowInstalled wfinstalled, bool defautFirstStepName = false)
        {
            if (wfinstalled == null) return "";
        ...
        /// 保存表单数据
        /// </summary>
        public string SaveFromData(string instanceid, BizProcess.Data.Model.WorkFlowCustomEventParams eventParams)
        {
            //保存自定义表单内容
            ...
            BizProcess.Data.Model.DBConnection dbconn = bdbconn.Get(dbconnid.ToGuid());
            if (dbconn == null)
            {
                return instanceid;
            }
            ...
            using (System.Data.IDbConnection conn = bdbconn.GetConnection(dbconn))
            {
                ...
                catch (Exception ex)
                {
                    ...
                    BizProcess.Platform.Log.Add(ex);
                }
                ...
                else
                {
                    ...
                    { 
                        ...
                        case "Oracle":
                            sql = string.Format("SELECT * FROM {0} WHERE {1}=:pk", dbtable, dbtablepk);
                            parList.Add(new Oracle.DataAccess.Client.OracleParameter(":pk", instanceid));
                            break;
                        case "MySql":
                            sql = string.Format("SELECT * FROM {0} WHERE {1}=@pk", dbtable, dbtablepk);
                ...
                switch (dbconn.Type)
                {
                    case "SqlServer":
                        ...
                        break;
                    case "Oracle":
                        Oracle.DataAccess.Client.OracleCommandBuilder ocb = new Oracle.DataAccess.Client.OracleCommandBuilder((Oracle.DataAccess.Client.OracleDataAdapter)dataAdapter);
                        break;
                    case "MySql":
                        MySql.Data.MySqlClient.MySqlCommandBuilder mcb = new MySql.Data.MySqlClient.MySqlCommandBuilder((MySql.Data.MySqlClient.MySqlDataAdapter)dataAdapter);
                        break;
                }
                ...
                if (isNew && isIdentity)
                {
                    switch (dbconn.Type)
                    {
                        ...
                        case "Oracle":
                            ...
                            using (Oracle.DataAccess.Client.OracleCommand cmd = new Oracle.DataAccess.Client.OracleCommand(identitysql1, (Oracle.DataAccess.Client.OracleConnection)conn))
                            {
                                object obj = cmd.ExecuteScalar();
                                if (obj != null)
                                {
                            ...
                            break;
                        case "MySql":
                            string identitysql2 = "SELECT @@IDENTITY";
                            using (MySql.Data.MySqlClient.MySqlCommand cmd = new MySql.Data.MySqlClient.MySqlCommand(identitysql2, (MySql.Data.MySqlClient.MySqlConnection)conn))
                            {
                                object obj = cmd.ExecuteScalar();
                                if (obj != null)
                                {
                ...
                foreach (string flowSubTableID in flowSubTableIDArray)
                {
                    ...
                    { 
                        ...
                        case "Oracle":
                            subSql = string.Format("SELECT * FROM {0} WHERE {1}=:pk", secondtable, secondtablerelationfield);
                            parList1.Add(new Oracle.DataAccess.Client.OracleParameter(":pk", primyarTableFeldValue));
                            break;
                        case "MySql":
                            subSql = string.Format("SELECT * FROM {0} WHERE {1}=@pk", secondtable, secondtablerelationfield);
                    ...
                    switch (dbconn.Type)
                    {
                        case "SqlServer":
                            ...
                            break;
                        case "Oracle":
                            Oracle.DataAccess.Client.OracleCommandBuilder ocb1 = new Oracle.DataAccess.Client.OracleCommandBuilder((Oracle.DataAccess.Client.OracleDataAdapter)dataAdapter1);
                            break;
                        case "MySql":
                            MySql.Data.MySqlClient.MySqlCommandBuilder mcb1 = new MySql.Data.MySqlClient.MySqlCommandBuilder((MySql.Data.MySqlClient.MySqlDataAdapter)dataAdapter1);
                            break;
                    }
        ...
        public LitJson.JsonData GetFormData(string connid, string table, string pk, string instanceid, string filedStatus = "")
        {
            ...
            BizProcess.Platform.DBConnection bdbconn = new BizProcess.Platform.DBConnection();
            BizProcess.Data.Model.DBConnection dbconn = bdbconn.Get(connid.ToGuid());
            if (dbconn == null)
            {
                return "";
            }
            ...
            using (System.Data.IDbConnection conn = bdbconn.GetConnection(dbconn))
            {
                ...
                switch (dbconn.Type)
                {
                    ...
                    case "Oracle":
                        sql = string.Format("SELECT * FROM {0} WHERE {1}=:fieldvalue {2}", secondTable, relationField,
                                (sortField.IsNullOrEmpty() ? "" : string.Concat("ORDER BY ", sortField)));
                        parList.Add(new Oracle.DataAccess.Client.OracleParameter(":fieldvalue", fieldValue));
                        break;
                    case "MySql":
                        sql = string.Format("SELECT * FROM {0} WHERE {1}=@fieldvalue {2}", secondTable, relationField,
                                (sortField.IsNullOrEmpty() ? "" : string.Concat("ORDER BY ", sortField)));
        ...
        /// 查询所有记录
        /// </summary>
        public List<BizProcess.Data.Model.WorkFlow> GetByTypes(string typeString)
        {
