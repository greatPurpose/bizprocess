...
namespace BizProcess.Platform
{
    ...
    {
        private BizProcess.Data.Interface.IDictionary dataDictionary;
        private static string cacheKey = BizProcess.Utility.Keys.CacheKeys.Dictionary.ToString();
        public Dictionary()
        {
        ...
        /// 查询所有记录
        /// </summary>
        public List<BizProcess.Data.Model.Dictionary> GetAll(bool fromCache=false)
        {
            if (!fromCache)
            {
            ...
            else
            {
                ...
                if (obj != null && obj is List<BizProcess.Data.Model.Dictionary>)
                {
                    return obj as List<BizProcess.Data.Model.Dictionary>;
                }
                else
                {
        ...
        /// 查询单条记录
        /// </summary>
        public BizProcess.Data.Model.Dictionary Get(Guid id, bool fromCache = false)
        {
        ...
        /// 查询下级记录
        /// </summary>
        public List<BizProcess.Data.Model.Dictionary> GetChilds(Guid id, bool fromCache=false)
        {
        ...
        /// 查询下级记录
        /// </summary>
        public List<BizProcess.Data.Model.Dictionary> GetChilds(string code, bool fromCache=false)
        {
            return code.IsNullOrEmpty() ? new List<BizProcess.Data.Model.Dictionary>() :
                fromCache ? getChildsByCodeFromCache(code) :
                dataDictionary.GetChilds(code.Trim());
        }

        private List<BizProcess.Data.Model.Dictionary> getChildsByCodeFromCache(string code)
        {
            var list = GetAll(true);
            var dict = list.Find(p => string.Compare(p.Code, code, true) == 0);
            return dict == null ? new List<BizProcess.Data.Model.Dictionary>() : list.FindAll(p => p.ParentID == dict.ID).OrderBy(p=>p.Sort).ToList();
        }

        private List<BizProcess.Data.Model.Dictionary> getChildsByIDFromCache(Guid id)
        {
            var list = GetAll(true);
        ...
        /// <returns></returns>
        public List<BizProcess.Data.Model.Dictionary> GetAllChilds(string code, bool fromCache)
        {
            if (code.IsNullOrEmpty()) return new List<BizProcess.Data.Model.Dictionary>();
            var dict = GetByCode(code, fromCache);
            if (dict == null) return new List<BizProcess.Data.Model.Dictionary>();
        ...
        /// <param name="fromCache">是否使用缓存</param>
        /// <returns></returns>
        public List<BizProcess.Data.Model.Dictionary> GetAllChilds(Guid id, bool fromCache=false)
        {
        ...
        private void addChilds(List<BizProcess.Data.Model.Dictionary> list, Guid id, bool fromCache=false)
        {
        ...
        /// <param name="fromCache">是否使用缓存</param>
        /// <returns></returns>
        public BizProcess.Data.Model.Dictionary GetByCode(string code, bool fromCache=false)
        {
        ...
        /// <returns></returns>
        private int getParentCount(List<BizProcess.Data.Model.Dictionary> dictList, BizProcess.Data.Model.Dictionary dict)
        {
            int parent = 0;
            BizProcess.Data.Model.Dictionary parentDict = dictList.Find(p => p.ID == dict.ParentID);
        ...
        private string getRadios(List<BizProcess.Data.Model.Dictionary> childs, string name, OptionValueField valueField = OptionValueField.Value, string value = "", string attr = "")
        {
            StringBuilder options = new StringBuilder(childs.Count * 100);
            foreach (var child in childs)
            {
        ...
        private string getCheckboxs(List<BizProcess.Data.Model.Dictionary> childs, string name, OptionValueField valueField = OptionValueField.Value, string value = "", string attr = "")
        {
            StringBuilder options = new StringBuilder(childs.Count * 100);
            foreach (var child in childs)
            {
        ...
        private string getOptionsValue(OptionValueField valueField, BizProcess.Data.Model.Dictionary dict)
        {
        ...
        {
            BizProcess.Cache.IO.Opation.Set(cacheKey, GetAll());
        }
