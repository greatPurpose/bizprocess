...
namespace BizProcess.Data.Oracle
{
    ...
    public class DBHelper
    {
        ...
        public DBHelper()
        {
            this.connectionString = BizProcess.Utility.Config.PlatformConnectionStringORACLE;
        }
        ...
        /// <param name="sql"></param>
        /// <returns></returns>
        public string GetPaerSql(string table, string fields, string where, string order, int size, int number, out long count, OracleParameter[] param = null)
        {
            ...
            string sql = string.Format("select {0},ROWNUM as PagerAutoRowNumber from {2} {3} ORDER BY {1}", fields.Equals("*") ? table + ".*" : fields, order, table, where2);
            ...
            StringBuilder sql1 = new StringBuilder();
            sql1.AppendFormat("select {0} from (", fields.IsNullOrEmpty() ? "*" : fields);
            sql1.Append(sql);
            sql1.AppendFormat(") PagerTempTable");
            if (count > size)
            {
